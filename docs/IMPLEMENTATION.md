...
本文记录 HyperDragShare `1.8.6`（`versionCode 81`）的当前完整实现、关键兼容性选择和已验证
...
根本不会加载模块。这条判定写在纯函数 `portalInjectionState()` 里，且优先级高于一切进程上报：刚被移出
...
当前 98 个单元测试覆盖：当前版本注入握手与限定服务启动命令、检测项的来源顺序/“检测中”占位/
...
触摸设备发现、握手只读传送门自己的 provider 而不碰其他组件、作用域不含传送门时注入行优先判为未注入、传送门未安装时直接探测返回“问不出来”
