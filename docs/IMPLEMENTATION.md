...
| LSPosed 注入 | `getRunningTargets()` 中传送门进程（`com.miui.contentextension` 或其 `:` 子进程）的 `HookedTarget.getState()`：任一进程为 `UP_TO_DATE` 即已注入当前版本；只有 `STALE`/`RELOADING`/`FAILED` 时为旧版本仍在运行（框架可能正在热重载）；列表为空为传送门未运行。**不要**用 `loadedVersionCode` 判定 —— API 文档写明它只是诊断值，框架可能用更强的代码标识判断新旧，比较版本号会把仍跑旧代码的进程说成已是当前版本 |
...
根本不会加载模块。实测 LSPosed 2.1.1 在模块 APK 被替换后会把该进程直接从 `getRunningTargets()` 里**移除**，而不是留下
`STALE`：所以“传送门还活着但跑的是旧模块”在框架眼里等于“没有目标”。因此每份上报都带上
`EXTRA_PORTAL_PID`（`ModuleActivation.reportPortalStatus()` 统一塞入），版本不匹配的上报也会记进
`last_report_version`/`last_report_pid`。检测时若最后一份上报来自别的版本、而那个 pid 仍在
`pidof com.miui.contentextension` 里，结论就是旧版本仍在运行 —— 不启动任何东西，也不等一份旧代码
再也发不出的上报（旧代码上报的是旧 `versionCode`，Provider 按约束拒收）。这台设备上换版本号安装会
顺带杀掉传送门进程，于是走的是“传送门未运行 → 冷启动 → 已注入当前版本”这条更快的路。

作用域判定写在纯函数 `portalInjectionState()` 里，且优先级高于一切进程上报：刚被移出
作用域时，之前启动的传送门进程仍带着模块，`getRunningTargets()` 照样报出当前 `loadedVersionCode`，
但那台宿主不会再加载模块。状态卡的总结论也由该行推出（`portalInjection == Injected && 传送门 Root`），
因此“模块作用域”显示缺少传送门时绝不会出现“已激活”。此时也不再冷启动传送门 —— 框架不会把模块
加载进去，等上报同样是白等。

“LSPosed 注入”在框架回应时**只**听框架：存量上报会比产生它的进程活得久，用它判定会把已经退出的
传送门说成已注入。框架没有回应时才回退到存量上报。

“传送门 Root 权限”不再依赖传送门进程。授权属于 UID，所以模块进程用 root 降到传送门 UID 再让 `su`
在那里决定即可：`su <portalUid> -c 'id -u; su -c id -u || true'`，第一行回显 UID 证明降权真的发生了，
后面出现 `0` 即为已授权，只有回显不对（某些 root 方案的 `su <uid>` 写法不同）才算“问不出来”。
这条命令和模块自身的 Root 探测并发执行，因此不额外增加耗时，也不需要传送门在后台。问不出来时才
退回传送门自己的上报。
...
`12 s`（冷启动传送门需要几秒）。上面的直接探测问不出来时才需要等传送门自己的上报：首份报告最多等
`6 s`，而“复核一个已经显示在卡片上的答案”只等 `2 s` —— 复核请求走传送门已经在监听的设置 URI
（`ModuleActivation.requestPortalRootReprobe()` → 观察者里再跑一次 `probePortalRootAccessAsync()`），
而被系统冻结（cached app freezer）的传送门进程可能几秒后才收到这个通知，所以超时后保留卡片上的
上一个答案而不是把整张卡挂住。每份 root 上报都会递增 `portal_root_sequence`，模块靠它区分新答案和
存量答案：值本身不变也算回答过，等的是这次往返而不是数值变化。因为观察者也是这条复核通道，它
故意活得比文本服务久——服务销毁后不再注销，只在进程结束时消失。若注入
报告始终没来，就用 root `pidof com.miui.contentextension` 判断传送门是否根本没起来，从而把
“传送门未运行”和“未注入”分开显示。这一步在框架回答过时同样要做：框架的 `getRunningTargets()` 只列出
装载了本模块的进程，所以“传送门在跑但没装载模块”和“传送门没在跑”在框架眼里长得一样，只有 `pidof`
能分开，且它是在尝试启动之后看的，比框架那次快照更新，因此结论优先级高于框架。握手期间已经由框架回答
