/*
 * Adapted from XiaomiHelper's compose drag-and-drop list implementation.
 * The list owns the pointer stream so items can move and auto-scroll while dragging.
 */
...
internal fun rememberDragDropState(
    ...
    LaunchedEffect(state) {
        while (true) {
            lazyListState.scrollBy(state.scrollChannel.receive())
        }
    }
    return state
}
...
internal class DragDropState internal constructor(
    ...
    private val draggingItemLayoutInfo: LazyListItemInfo?
        get() = state.layoutInfo.visibleItemsInfo.firstOrNull {
            ...
            val overscroll = when {
                ...
                draggingItemDraggedDelta < 0 ->
                    (startOffset - state.layoutInfo.viewportStartOffset -
                        state.layoutInfo.beforeContentPadding).coerceAtMost(0f)
                else -> 0f
            }
            if (overscroll != 0f) {
